$(function() {
  ...
  let item = '';                                 // Элемент является пустой строкой
  ...
  function updateCount() {                       // Создаем функцию для обновления счетчика
    let items = $('li[class!=complete]').length; // Количество элементов в списке
  ...
  $newItemForm.on('submit', function(e) {       // При отправке нового пункта
    ...
    let text = $('input:text').val();           // Получаем значение текстового поля
  ...
  $list.on('click', 'li', function() {
    let $this = $(this);               // Кэшируем элемент в объект jQuery
    let complete = $this.hasClass('complete');  // Является ли пункт завершенным
